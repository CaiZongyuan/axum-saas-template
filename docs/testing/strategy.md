...
这是 [架构规范](../saas-template-architecture-spec.md) 的测试实施方案。当前已实现 T01 的状态链路，实际命令与覆盖见 [T01 测试反馈循环](t01-feedback-loop.md)。下文同时定义后续功能的验证入口与责任，不代表全部 v1 测试已存在或通过。
...
5. 修改过程中常跑定向测试和类型检查；完成时运行 `just check`。按用户确认的节奏，浏览器 E2E 集中在关键旅程完成、浏览器回归或里程碑验收时运行，避免本地、每次 push、合并后重复同一套旅程；记录对应提交和结果。
